...
function hoot_archive_title_filter( $title ) {
	...
	elseif ( is_category() )
		$title = str_replace( __( 'Category: ', 'nevark' ), '', $title );

	elseif ( is_tag() )
		$title = str_replace( __( 'Tag: ', 'nevark' ), '', $title );

	elseif ( is_tax() && !is_tax( 'post_format' ) )
		$title = single_term_title( '', false );

	elseif ( is_author() )
		$title = str_replace( __( 'Author: ', 'nevark' ), '', $title );
	...
	elseif ( is_post_type_archive() )
		$title = str_replace( __( 'Archives: ', 'nevark' ), '', $title );
	...
	if ( ( ( $page = get_query_var( 'paged' ) ) || ( $page = get_query_var( 'page' ) ) ) && $page > 1 ) {
		...
		$suffix = ' <span class="loop-title-suffix">' . sprintf( __( '(Page %s)', 'nevark' ), $paged ) . '</span>';
