			...
			foreach ( $sections as $key => $section ) :
				if ( empty( $section[ 'sortitem_hide' ] ) ):
					...
					elseif ( $key == 'content' )
						...
						$type = $key;
					$type = apply_filters( 'nevark_frontpage_sections_switch', $type, $key, $sections );

					// Exit the loop except main content for a paged post list (blog)
					if ( $type != 'content-blog' && apply_filters( 'nevark_paged_frontpage_hidemodules', is_paged() ) )
						continue;
					...
					if ( $custom_template ):
						...
						switch( $type ):
							...
							case 'widgetarea':
								...
								if ( $displayarea ) : ?>
									<div id="frontpage-<?php echo sanitize_html_class( $areakey ); ?>" <?php hoot_attr( 'frontpage-area', $areakey, 'frontpage-area frontpage-widgetarea ' . esc_attr( $section_class ) ); ?>>
										<div class="hgrid">
											<?php
											for ( $c = 1; $c <= $count ; $c++ ) {
							...
							case 'content-blog':
								wp_reset_postdata(); ?>
								<div id="frontpage-page-content" <?php hoot_attr( 'frontpage-area', $key, 'frontpage-area frontpage-pagecontent ' . esc_attr( $section_class ) ); ?>>
									<?php
							...
							default:
								...
								do_action( 'nevark_frontpage_sections', $type, $sections, $section_class, $context );
